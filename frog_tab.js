treeJSON_xp = d3.json("frog.json", function(error, treeData) {
...
var svg = d3.select("#graph_xp").append("svg")
...
var timepoint_names = ["S8 ", "S10 ", "S11 ", "S12 ", "S13 ", "S14", "S16", "S18", "S20", "S22"];

var ul = d3.select("#graph_xp").append('ul')
...
function update(source) {
    ...
    nodes.forEach(function(d) { d.y = d.depth * 96; });
...
//        myMap.set("exe_germ", "#E67E22")
    ...
    var nodeEnter = node.enter().append("g")
        ...
        .attr("data-target", "#NodeModal_xp").style("cursor", "pointer") // display the bootstrap dialog
    ...
    link.enter().insert("path", "g")
        ...
        .attr("opacity", function(d){
            return d.target.edge_weight;})
        .attr("data-toggle", "modal") // display the bootstrap dialog
        .attr("data-target", "#EdgeModal_xp").style("cursor", "pointer") // display the bootstrap dialog
    ...
    legendCelltype = svg.append("g")
        ...
        .call(d3.legendCelltype_xp)
    // done

    //Init NodeTable_xp
    d3.select("#NodeTable_xp")
       .append("table")
    // done
    ...
    function clickNode(node_id) {
        ...
        // update the NodeTable_xp
        d3.tsv("NodeTable_xp.tsv", function(error, NodeData) {
            
            NodeArray = [];
            NodeData.forEach(function(d, i){// update the NodeTable_xp
                NodeArray.push([d.key, d[node_id]]);
            });
            // update the title of NodePage as well
            d3.select("#NodeTitle_xp")
                .text(NodeArray[0][1].replace(':', ' : '));

            //Add a new (updated) NodeTable_xp
            d3.select("#NodeTable_xp")
                .append("table")
    ...
    function clickLink(d) {
        ...
        d3.tsv("EdgeTable_xp.tsv", function(error, EdgeDataOrg) {
            ...
            d3.select("#EdgeTitle_xp")
                .text('Edge: ' + source_node_id + ' to ' + target_node_id);

            //Add a new (updated) EdgeTable_xp
            d3.select("#EdgeTable_xp")
              .append("table")
              .attr("style", "font-size: 15px")
              .append("tbody")
